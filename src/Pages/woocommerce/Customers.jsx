...
function Customers() {
  ...
  useEffect(() => {
    if (api) {
      ...
      fetch(`/wp-json/wc/v3/customers/?per_page=100`, requestOptions)
        .then((response) => response.json())
        .then((result) => {
            console.log(result)
            setCustomers(result)
        ...
        <div className="container-fluid">
          <div className="row">
                ...
                <div className="col-xl-12">
                  <div className="card">
                    <div className="card-header border-0">
                      <h4 className="heading mb-0">Customers without orders (dummy data)</h4>
                    </div>
            ...
            <tr key={customer.id}>
              ...
              <td>{customer.billing?.phone || "N/A"}</td>
              <td>{customer.billing?.company || "N/A"}</td>
              <td>{customer.billing?.address_1 || "N/A"}</td>
              <td>{customer.billing?.address_2 || "N/A"}</td>
              <td>{customer.billing?.city || "N/A"}</td>
              <td>{customer.billing?.state || "N/A"}</td>
              <td>{customer.billing?.country || "N/A"}</td>
              <td>{customer.billing?.postcode || "N/A"}</td>
